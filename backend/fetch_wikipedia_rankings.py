...
def main() -> None:
    ...
    as_of = ""
    for div, title in (("men", "Men's singles"), ("women", "Women's singles")):
        ...
        rows, div_asof = parse_section(html, args.top)
        as_of = as_of or div_asof
        result[div] = rows
        head = ", ".join(f"{r['rank']}.{r['name']}" for r in rows[:3])
        print(f"  ✓ {div}: {len(rows)} players (as of {div_asof or '?'}) — {head}…")

    if not (result.get("men") or result.get("women")):
        raise SystemExit("❌ parsed 0 players — the page layout may have changed.")

    out = {
        "_readme": ("Official BWF World Ranking (singles), parsed from "
                    "https://en.wikipedia.org/wiki/BWF_World_Ranking via the "
                    "MediaWiki API. Refresh with: python backend/"
                    "fetch_wikipedia_rankings.py . Names normalized to match "
                    "src/players.ts. Served by the backend at GET /rankings."),
